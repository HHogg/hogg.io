...
import isPointWithinCircle from '../utils/isPointWithinCircle';
 ...
 *
 */
export const pushIndexToTraversal = (
  { traversals, nodes, edges }: Graph,
  index: number
): Traversal[] => {
  ...
  if (currentTraversal) {
    ...
    return [
      ...
      appendEdgeToPath(currentTraversal, edges[index - nodes.length]),
    ];
  }
 ...
 *
 */
export const popIndexFromTraversal = ({ traversals }: Graph): Traversal[] => {
  const lastTraversal = getCurrentTraversal(traversals) || traversals[0];

  if (lastTraversal) {
    if (lastTraversal.path.length === 1) {
      return traversals.slice(0, -1);
    }

    return [
      ...traversals.slice(0, -1),
      removeLastNodeAndEdgeFromPath(lastTraversal),
    ];
  }
...
export const removeLastNodeAndEdgeFromPath = ({
  bitset,
  index,
  path,
}: Traversal): Traversal => {
  ...
  return {
    index: index,
    bitset: bitset.clone().set(edgeIndex, 1).set(nodeIndex, 0),
    isComplete: false,
    path: path.slice(0, -2),
  };
};
...
export const removeTraversal = (traversals: Traversal[], index: number) => {
  return [...traversals.slice(0, index), ...traversals.slice(index + 1)];
};
 ...
 *
 */
export const getTraversals = (
  circles: Circle[],
  nodes: Node[],
  edges: Edge[]
): Traversal[] => {
  let graph: Graph = getUpdatedGraphState({
    circles,
    nodes,
    edges,
    traversals: [],
    ...
    for (const edgeIndex of links[fromNodeIndex]) {
      if (!getCurrentTraversal(graph.traversals)) {
        graph = getUpdatedGraphState({
          ...graph,
          traversals: pushIndexToTraversal(graph, fromNodeIndex),
        });
      }
      ...
      if (graph.edges[edgeIndex - graph.nodes.length].state.isSelectable) {
        ...
        graph = getUpdatedGraphState({
          ...graph,
          traversals: pushIndexToTraversal(graph, edgeIndex),
        });
  ...
  return graph.traversals.filter(({ path }) => {
    ...
    for (const edge of edges) {
      for (
        let circleIndex = 0;
        circleIndex < graph.circles.length;
        circleIndex++
      ) {
        const { x: cx, y: cy, radius: cr } = graph.circles[circleIndex];

        if (
          circleIndex !== edge.circle &&
          isPointWithinCircle(edge.x, edge.y, cx, cy, cr)
        ) {
          return true;
        }
      }
