...
use crate::{build, hash, TilingError};
...
pub struct Tiling {
  ...
  option_hash_version: hash::Version,
...
impl Tiling {
  pub fn with_feature_toggles<const N: usize>(
    self,
    option_feature_toggles: [FeatureToggle; N],
  ) -> Self {
    self.with_feature_toggles_set(HashMap::from_iter(
      option_feature_toggles.iter().cloned().map(|ft| (ft, true)),
    ))
  }

  pub fn with_feature_toggles_set(
    mut self,
    option_feature_toggles: HashMap<FeatureToggle, bool>,
  ) -> Self {
    self.option_feature_toggles = option_feature_toggles;
    self
  }
  ...
  pub fn with_hash_version(mut self, option_hash_version: hash::Version) -> Self {
    self.option_hash_version = option_hash_version;
    self
  }
