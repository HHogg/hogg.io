...
#[serde(rename = "TilingsGetPagedRequest")]
...
async fn get_total(pool: &Pool<Postgres>, request: &Request) -> Result<i64> {
...
async fn get_rows(pool: &Pool<Postgres>, request: &Request) -> Result<Vec<Tiling>> {
  let Request {
    page,
    page_direction,
    page_size,
