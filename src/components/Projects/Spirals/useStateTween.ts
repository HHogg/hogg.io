...
export default (t: number, vectorsA: TypeVectorWithSize[]): [TypeVectorWithSize[], TypeVectorWithSize[]] => {
  ...
  return useMemo(() => {
    const vectors = vectorsA.map((_, i) =>
      refVectorsA.current[i].map((_, j) =>
        lerp(refVectorsA.current[i][j], refVectorsB.current[i][j], t))) as TypeVectorWithSize[];
